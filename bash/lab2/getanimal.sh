...
# where the keys are colours, and the values are animals who are that colour. Use the colours you put into the first array. 
# Ask the user for a number and use it to display a colour from the array, then the corresponding animal from the associative array for that colour. 
